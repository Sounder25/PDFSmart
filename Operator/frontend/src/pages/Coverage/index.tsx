    ...
    <div className="space-y-5">
      ...
      {logs.length === 0 ? (
        <div className="op-card p-12 text-center">
          ...
          <p className="text-xs text-slate-700 mt-1">Log every podcast, press hit, speaking gig, and media mention here</p>
      ...
      <Modal open={showAdd} onClose={() => { setShowAdd(false); setForm({ ...EMPTY }) }} title="Log Coverage" size="md">
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div><label className="op-label">Type</label>
              <select className="op-input" value={form.type} onChange={f('type')}>
                {COVERAGE_TYPES.map(t => <option key={t} value={t}>{TYPE_LABELS[t]}</option>)}
              </select>
            </div>
            <div><label className="op-label">Sentiment</label>
              <select className="op-input" value={form.sentiment} onChange={f('sentiment')}>
          ...
          <div><label className="op-label">Headline / Title</label><input className="op-input" value={form.headline} onChange={f('headline')} placeholder="Article headline, episode title, or talk title" /></div>
          <div className="grid grid-cols-2 gap-3">
