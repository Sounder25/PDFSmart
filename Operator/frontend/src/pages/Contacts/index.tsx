            ...
            <tbody>
              {contacts.map(c => (
                <tr key={c.id} className="border-b border-[#161f30] hover:bg-white/2 transition-colors">
                  ...
                  <td className="px-3 py-2.5">
                    <span className={cn('status-badge', CONTACT_TYPE_COLORS[c.type] || CONTACT_TYPE_COLORS.general)}>
                  ...
                  <td className="px-3 py-2.5 text-slate-500">
                    {c.location ? <span className="flex items-center gap-1"><MapPin className="w-2.5 h-2.5" />{c.location}</span> : '—'}
                  </td>
                  <td className="px-3 py-2.5">
                    <span className={cn('status-badge', CONTACT_STATUS_COLORS[c.status] || CONTACT_STATUS_COLORS.cold)}>
                      {c.status}
                    </span>
                  </td>
