      ...
      {investors.length > 0 && (
        <div className="grid grid-cols-4 gap-3">
          <div className="op-card p-4">
            <div className="text-2xl font-bold text-slate-100">{investors.length}</div>
            <div className="text-[10px] text-slate-500 uppercase tracking-wider mt-0.5">In Pipeline</div>
          </div>
          <div className="op-card p-4">
            <div className="text-2xl font-bold text-green-300">{formatCurrency(totalCommitted, true)}</div>
            <div className="text-[10px] text-slate-500 uppercase tracking-wider mt-0.5">Committed</div>
          </div>
          <div className="op-card p-4">
            <div className="text-2xl font-bold text-brand-300">{formatCurrency(totalAsk, true)}</div>
            <div className="text-[10px] text-slate-500 uppercase tracking-wider mt-0.5">Total Ask</div>
          </div>
          <div className="op-card p-4">
            <div className="text-2xl font-bold text-blue-300">{investors.filter(i => ['meeting_scheduled', 'follow_up', 'due_diligence'].includes(i.stage)).length}</div>
            <div className="text-[10px] text-slate-500 uppercase tracking-wider mt-0.5">Active Convos</div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          {!isLoading && investors.length === 0 && (
            <div className="op-card p-6 text-center">
              ...
              <p className="text-[10px] text-slate-700 mt-1">Add investor-type contacts first, then add them here</p>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    {selected.next_action_date && (
                      <span className="text-[10px] text-amber-400">Next: {formatDate(selected.next_action_date, true)}</span>
                    )}
                  </div>
                </div>
      ...
      <Modal open={showAdd} onClose={() => { setShowAdd(false); setNewContactId('') }} title="Add Investor to Pipeline" size="md">
        <div className="space-y-3">
          <div><label className="op-label">Contact *</label>
            ...
            {contactList.length === 0 && <p className="text-[10px] text-slate-600 mt-1">No investor-type contacts found. Add contacts with type "Investor" first.</p>}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="op-label">Stage</label>
              <select className="op-input" value={form.stage} onChange={f('stage')}>
                {STAGES.map(s => <option key={s} value={s}>{INVESTOR_STAGE_LABELS[s]}</option>)}
              </select>
            </div>
          ...
          <div><label className="op-label">Next Action</label><input className="op-input" value={form.next_action} onChange={f('next_action')} placeholder="e.g. Send pitch deck, Schedule call" /></div>
