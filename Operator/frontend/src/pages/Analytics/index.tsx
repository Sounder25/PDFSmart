    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="op-card p-4">
          <div className="text-2xl font-bold text-teal-300">{k?.outreach_total ?? 0}</div>
          <div className="text-[10px] text-slate-500 uppercase tracking-wider mt-0.5">Total Outreach</div>
          <div className="text-[10px] text-slate-600 mt-1">{responseRate}% response rate</div>
        </div>
        <div className="op-card p-4">
          <div className="text-2xl font-bold text-purple-300">{k?.coverage_count ?? 0}</div>
          <div className="text-[10px] text-slate-500 uppercase tracking-wider mt-0.5">Media Coverage</div>
          <div className="text-[10px] text-slate-600 mt-1">~{((k?.total_reach ?? 0) / 1000).toFixed(0)}K total reach</div>
        </div>
        <div className="op-card p-4">
          <div className="text-2xl font-bold text-emerald-300">{formatCurrency(k?.total_revenue)}</div>
          <div className="text-[10px] text-slate-500 uppercase tracking-wider mt-0.5">Total Revenue</div>
          <div className="text-[10px] text-slate-600 mt-1">{formatCurrency(k?.book_revenue, true)} from book</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="op-card p-4 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-200">Contacts by Type</h2>
          </div>
          <div className="space-y-3">
            {byType.length === 0 && <p className="text-xs text-slate-700 text-center py-4">No contacts yet</p>}
            {byType.map(({ type, count }) => (
              <Bar key={type} label={CONTACT_TYPE_LABELS[type] || type} value={count} max={maxByType} color="bg-teal-500/60" />
            ))}
          </div>
        </div>
        ...
        <div className="op-card p-4 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-200">Investor Pipeline</h2>
          </div>
          <div className="space-y-3">
            ...
            {invSummary.map(s => (
              <div key={s.stage} className="space-y-1">
                ...
                <div className="h-2 bg-slate-800/60 rounded-full overflow-hidden">
                  <div className="h-full rounded-full bg-green-500/50 transition-all" style={{ width: `${Math.round((s.count / maxInvStage) * 100)}%` }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            ...
            {revSummary.map(s => (
              <div key={s.type} className="space-y-1">
                <div className="flex justify-between text-xs">
                  <span className="text-slate-400">{s.type.replace(/_/g, ' ')}</span>
                  <span className="text-emerald-300 font-medium">{formatCurrency(s.total, true)}</span>
                </div>
                <div className="h-2 bg-slate-800/60 rounded-full overflow-hidden">
                  <div className="h-full rounded-full bg-emerald-500/60 transition-all" style={{ width: `${Math.round((s.total / maxRev) * 100)}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="op-card p-4 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-200">Outreach Summary</h2>
          </div>
          <div className="space-y-3">
