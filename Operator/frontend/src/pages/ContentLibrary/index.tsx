...
export function ContentLibrary() {
  ...
  const f = (k: string) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | HTMLInputElement>) => setForm(p => ({ ...p, [k]: e.target.value }))
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        {ASSET_TYPES.map(t => (
          <button key={t} onClick={() => setFilter(t)} className={cn('btn-ghost text-xs py-1 px-2.5', filter === t && 'bg-white/8 text-slate-200')}>
            {TYPE_LABELS[t]}
          </button>
        ))}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="op-card p-12 text-center">
          ...
          <p className="text-xs text-slate-700 mt-1">Store your email templates, pitch decks, press releases, and more</p>
            ...
            <div key={a.id} className="op-card p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  {a.content && (
                    <p className="text-xs text-slate-500 mt-2 line-clamp-2 whitespace-pre-wrap">{a.content.slice(0, 200)}{a.content.length > 200 ? '…' : ''}</p>
                  )}
                </div>
                <div className="flex gap-1 shrink-0">
                  {a.content && (
                    <button className="btn-ghost p-1.5" onClick={() => copy(a.content!, a.id)} title="Copy content">
      ...
      <Modal open={showAdd || !!editing} onClose={() => { setShowAdd(false); setEditing(null); setForm({ ...EMPTY }) }} title={editing ? 'Edit Asset' : 'New Content Asset'} size="lg">
        <div className="space-y-3">
          <div><label className="op-label">Title *</label><input className="op-input" value={form.title} onChange={f('title')} placeholder="e.g. Investor Intro Email — V2" /></div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="op-label">Audience</label>
              <select className="op-input" value={form.audience} onChange={f('audience')}>
          ...
          <div><label className="op-label">Notes</label><input className="op-input" value={form.notes} onChange={f('notes')} placeholder="Version notes, context, usage guidance…" /></div>
