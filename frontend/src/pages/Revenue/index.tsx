    ...
    <div className="space-y-5">
              ...
              {events.map(e => {
                ...
                return (
                  <tr key={e.id} className="border-b border-[#161f30] hover:bg-white/2 transition-colors">
                    <td className="px-3 py-2.5"><div className="flex items-center gap-1.5"><Icon className="w-3.5 h-3.5 text-emerald-400/60" /><span className="text-slate-400">{REVENUE_TYPE_LABELS[e.type] || e.type}</span></div></td>
                    ...
                    <td className="px-3 py-2.5"><button className="btn-ghost p-1 text-red-400/40 hover:text-red-400" onClick={() => setDeleting(e)}><Trash2 className="w-3.5 h-3.5" /></button></td>
                  </tr>
                )
              })}
            </tbody>
      ...
      <Modal open={showAdd} onClose={() => { setShowAdd(false); setForm({ ...EMPTY }) }} title="Record Revenue" size="sm">
        <div className="space-y-3">
          <div><label className="op-label">Type</label><select className="op-input" value={form.type} onChange={f('type')}>{REVENUE_TYPES.map(t => <option key={t} value={t}>{REVENUE_TYPE_LABELS[t]}</option>)}</select></div>
          ...
          <div><label className="op-label">Description</label><input className="op-input" value={form.description} onChange={f('description')} /></div>
