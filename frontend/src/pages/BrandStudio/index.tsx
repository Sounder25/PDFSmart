    ...
    <div className="space-y-5">
      ...
      {tab === 'mission' && (
        <div className="op-card p-5 space-y-5">
          ...
          <EditableField label="The Problem (what we're losing)" value={brand?.problem_statement} onSave={v => save({ problem_statement: v })} multiline placeholder="Describe the problem" />
          <EditableField label="The Solution (what you bring)" value={brand?.solution_statement} onSave={v => save({ solution_statement: v })} multiline placeholder="How does your tech change the game?" />
          <EditableField label="Origin Story" value={brand?.origin_story} onSave={v => save({ origin_story: v })} multiline placeholder="How did you get here?" />
          <div className="grid grid-cols-2 gap-5">
      ...
      {tab === 'book' && (
        <div className="op-card p-5 space-y-5">
          ...
          <EditableField label="Book Description / Pitch" value={brand?.book_description} onSave={v => save({ book_description: v })} multiline placeholder="What's the book about?" />
      ...
      {tab === 'audiences' && (
        <div className="op-card p-5 space-y-5">
          ...
          <div className="border-t border-[#1f2d40] pt-5">
            <EditableField label="Call to Action — Investors" value={brand?.cta_investor} onSave={v => save({ cta_investor: v })} multiline placeholder="What do you want investors to do?" />
          </div>
          <EditableField label="Call to Action — Customers" value={brand?.cta_customer} onSave={v => save({ cta_customer: v })} multiline placeholder="What do you want farmers/ranchers to do?" />
          <EditableField label="Call to Action — Readers" value={brand?.cta_reader} onSave={v => save({ cta_reader: v })} multiline placeholder="What do you want book readers to do?" />
        </div>
      )}
